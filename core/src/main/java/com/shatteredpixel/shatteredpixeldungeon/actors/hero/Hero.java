...
public class Hero extends Char {
    ...
    public boolean shoot(Char enemy, float dmgMulti, float dmgBonus, float accMulti) {

        this.enemy = enemy;

        boolean hit = shootAttack(enemy, dmgMulti, dmgBonus, accMulti);
        Invisibility.dispel();
    ...
    public int shootDamageRoll() {
        ...
        if (wep instanceof Gun) {
            dmg = ((Gun) wep).shootDamageRoll(this);
        } else {
            dmg = damageRoll();
        }
        if (dmg < 0) dmg = 0;
